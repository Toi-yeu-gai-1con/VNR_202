...
const campaign = { health: 21, stamina: 43, saDoa: 37, levelId: "hub" };
const session = createTrainingSession(campaign);

assert.deepEqual(session.enter(), { health: 36, stamina: 100, saDoa: 0, levelId: "training" }, "Training starts with a safe independent combat state.");
session.takeHit(9);
session.addCorruption(20);
assert.deepEqual(session.reset(), { health: 36, stamina: 100, saDoa: 0, levelId: "training" }, "Reset restores the safe training snapshot only.");
