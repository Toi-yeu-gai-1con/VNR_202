...
function loadTvaEmployeeSpriteSet() {
  return {
    down: loadSprite("assets/npcs/tva-employee/down.png"),
    downleft: loadSprite("assets/npcs/tva-employee/downleft.png"),
    left: loadSprite("assets/npcs/tva-employee/left.png"),
    up: loadSprite("assets/npcs/tva-employee/up.png"),
    upleft: loadSprite("assets/npcs/tva-employee/upleft.png"),
  };
}
...
function renderBadEndingRecoveryScene(context, width, height, scene) {
  ...
  const actorOpacity = frame.phase === "reset" ? 1 - easeInOutCubic(frame.phaseProgress) : 1;
...
function updateMonsters(deltaSeconds) {
  for (const monster of currentLevel().monsters ?? []) {
    ...
    if (monster.telegraphEndsAt && state.lastTimestamp >= monster.telegraphEndsAt) {
      ...
      monster.attackEndsAt = state.lastTimestamp + (monster.attackAnimationMs ?? MONSTER_ATTACK_ANIMATION_MS);
      monster.telegraphStartsAt = 0;
      monster.telegraphEndsAt = 0;
...
function drawMonsterSprite(monster, hitFlash) {
  ...
  const isAttacking = state.lastTimestamp < (monster.attackEndsAt ?? 0);
  const animationKey = isAttacking && config?.animations?.attack
    ? "attack"
    : monster.animationState === "run"
      ? "run"
      : "idle";
  ...
  const animationElapsed = animationKey === "attack"
    ? Math.max(0, state.lastTimestamp - (monster.attackStartedAt ?? state.lastTimestamp))
    : state.lastTimestamp + (monster.phase ?? 0) * 1000;
  const frameIndex = animationKey === "attack"
    ? Math.min(animation.frameCount - 1, Math.floor(animationElapsed / animation.frameDuration))
    : Math.floor(animationElapsed / animation.frameDuration) % animation.frameCount;
  const attackProgress = isAttacking ? getTimedProgress(monster.attackStartedAt, monster.attackEndsAt) : 0;
...
function getMonsterSpriteDirection(monster, config = null) {
  ...
  const direction = isEngaged
    ...
    : monster.moveDirection ?? monster.attackDirection ?? getDirectionFromVector(player.x - monster.x, player.y - monster.y);
